...
- 自动识别 ML307A、ML307C、ML307Y 和上游 ML307R 的完整料号；优先使用 PDU 直接上报，旧固件不支持时自动降级为 SIM 存储通知
- 运营商扫描、手动选网或恢复自动选网结束后会自动恢复 `CMGF/CNMI`，修复部分 ML307 固件在 `COPS` 操作后静默关闭短信上报的问题
...
- 支持长短信自动合并（30秒超时）
- 支持管理员短信远程发送短信和重启设备
- 提供 Windows 批量量产工具，可并行烧录、自动验收模组/SIM/驻网状态并输出 CSV 和逐台 JSON 记录
...
固件会组合 `ATI`、`AT+CGMM`、`AT+CGMR` 动态识别 ML307A/C/R/Y，并先探测能力再决定是否执行 PDP 开关。短信上报优先配置 `CNMI=2,2`，不兼容时自动改用 `CNMI=2,1` 和已有的安全读删队列。运营商支持最终由**完整模组料号、射频频段、模组固件和当地网络**共同决定，ESP32 固件无法为缺少频段的硬件增加“全网通”能力。
...
Windows 量产工具位于 [`factory/`](factory/README.md)。执行 `factory/build_firmware.ps1` 生成带 SHA-256 清单的固件包，然后双击 `factory/start.bat` 即可多串口并行烧录。每台板会通过 USB 只读工厂协议核对 ESP32-C3、固件版本、ML307 完整料号、短信配置，并可选择强制要求 SIM 和驻网通过。GitHub Actions 同时提供免 Python 的 Windows 单文件程序和配套固件包。
